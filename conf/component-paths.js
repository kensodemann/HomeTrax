/*jshint node: true */

module.exports = {
  css: {
    angularMotion: {
      dev: 'bower_components/angular-motion/dist/angular-motion.css',
      release: 'bower_components/angular-motion/dist/angular-motion.min.css'
    },
    bootstrapAdditions: {
      dev: 'bower_components/bootstrap-additions/dist/bootstrap-additions.css',
      release: 'bower_components/bootstrap-additions/dist/bootstrap-additions.min.css',
    },
    fullcalendar: {
      dev: 'bower_components/fullcalendar/dist/fullcalendar.css',
      release: 'bower_components/fullcalendar/dist/fullcalendar.min.css'
    }
  },
  lib: {
    jquery: {
      dev: 'bower_components/jquery/dist/jquery.js',
      release: 'bower_components/jquery/dist/jquery.min.js'
    }
    ,
    angular: {
      dev: 'bower_components/angular/angular.js',
      release: 'bower_components/angular/angular.min.js'
    }
    ,
    bootstrap: {
      dev: 'bower_components/bootstrap/dist/js/bootstrap.js',
      release: 'bower_components/bootstrap/dist/js/bootstrap.min.js'
    }
    ,
    angularAnimate: {
      dev: 'bower_components/angular-animate/angular-animate.js',
      release: 'bower_components/angular-animate/angular-animate.js'
    }
    ,
    angularLocalStorage: {
      dev: 'bower_components/angular-local-storage/dist/angular-local-storage.js',
      release: 'bower_components/angular-local-storage/dist/angular-local-storage.min.js'
    }
    ,
    angularMessages: {
      dev: 'bower_components/angular-messages/angular-messages.js',
      release: 'bower_components/angular-messages/angular-messages.min.js'
    }
    ,
    angularResource: {
      dev: 'bower_components/angular-resource/angular-resource.js',
      release: 'bower_components/angular-resource/angular-resource.min.js'
    }
    ,
    angularRoute: {
      dev: 'bower_components/angular-route/angular-route.js',
      release: 'bower_components/angular-route/angular-route.min.js'
    }
    ,
    angularStrap: {
      dev: 'bower_components/angular-strap/dist/angular-strap.js',
      release: 'bower_components/angular-strap/dist/angular-strap.min.js'
    }
    ,
    angularStrapTpl: {
      dev: 'bower_components/angular-strap/dist/angular-strap.tpl.js',
      release: 'bower_components/angular-strap/dist/angular-strap.tpl.min.js'
    }
    ,
    angularTypeahead: {
      dev: 'bower_components/angular-typeahead/angular-typeahead.js',
      release: 'bower_components/angular-typeahead/angular-typeahead.min.js'
    }
    ,
    angularUiCalendar: {
      dev: 'bower_components/angular-ui-calendar/src/calendar.js',
      release: 'bower_components/angular-ui-calendar/src/calendar.js'
    }
    ,
    angularMocks: {
      dev: 'bower_components/angular-mocks/angular-mocks.js'
    }
    ,
    bloodhound: {
      dev: 'bower_components/typeahead.js/dist/bloodhound.js',
      release: 'bower_components/typeahead.js/dist/bloodhound.min.js'
    }
    ,
    fullCalendar: {
      dev: 'bower_components/fullcalendar/dist/fullcalendar.js',
      release: 'bower_components/fullcalendar/dist/fullcalendar.min.js'
    }
    ,
    moment: {
      dev: 'bower_components/moment/moment.js',
      release: 'bower_components/moment/min/moment.min.js'
    }
    ,
    toastr: {
      dev: 'bower_components/toastr/toastr.js',
      release: 'bower_components/toastr/toastr.min.js'
    }
    ,
    typeahead: {
      dev: 'bower_components/typeahead.js/dist/typeahead.jquery.js',
      release: 'bower_components/typeahead.js/dist/typeahead.jquery.min.js'
    }
    ,
    underscore: {
      dev: 'bower_components/underscore/underscore.js',
      release: 'bower_components/underscore/underscore-min.js'
    }
  }
  ,
  fonts: {
    fontAwesome: 'bower_components/fontawesome/fonts',
    bootstrap: 'bower_components/bootstrap-sass/assets/fonts'
  }
}
;
